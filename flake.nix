{
  ...
  inputs = {
    ...
    # PiKVM daemon + the RPi kernel patches zerokvm's kernel is built with
    # (see machines/zerokvm/hw-config.nix). Only zerokvm consumes this.
    # Note that its revision feeds the kernel derivation hash, so bumping it
    # forces a kernel recompile — which must happen on a real machine, never
    # on the Pi Zero 2 W itself (4 cores, ~400MiB RAM).
  ...
  in {
    ...
    nixosConfigurations.zerokvm = inputs.nixpkgs.lib.nixosSystem {
      system = "aarch64-linux";
      modules = [
        ./machines/zerokvm
        ./machines/zerokvm/hw-config.nix
      ];
      specialArgs = { inherit inputs; };
    };
    ...
    deploy.nodes.zerokvm = {
      hostname = "zerokvm.lion-zebra.ts.net";
      profiles.system = {
        user = "root";
        path = inputs.deploy-rs.lib.aarch64-linux.activate.nixos self.nixosConfigurations.zerokvm;
      };
      sshUser = "ritiek";
    };
