...
in
{
  ...
  networking.nat = {
    enable = true;
    externalInterface = wanPpp;
    internalInterfaces = [ lanBridge ];
    # No IPv6 NAT: DHCPv6-PD (see lan.nix) gives LAN clients real globally
    # routable addresses.
    enableIPv6 = false;
  };
  ...
  networking.nftables.tables.router = {
    ...
    content = ''
      ...
      # ppp-wan it would expose an open recursive resolver -- a DNS
      ...
      chain mss-clamp {
        ...
        oifname "${wanPpp}" tcp flags syn / syn,rst tcp option maxseg size set rt mtu
        iifname "${wanPpp}" tcp flags syn / syn,rst tcp option maxseg size set rt mtu
      }
